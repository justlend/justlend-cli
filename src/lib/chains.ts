...
export interface JustLendAddresses {
  ...
  /** stUSDT staking related contracts (USDT → stUSDT, wstUSDT wrapper, withdrawal queue). */
  stUSDT: {
    usdt: string;        // USDT TRC20 (the asset deposited)
    stUsdt: string;      // StUSDTProxy
    wstUsdt: string;     // wstUSDTProxy (wrapped, non-rebasing)
    minter: string;      // minterProxy — receives USDT, mints stUSDT
    unstaker: string;    // UnstUSDTProxy — claim queue
    decimals: number;    // stUSDT / wstUSDT decimals (18)
    usdtDecimals: number; // USDT decimals (6)
  };
...
export const JUSTLEND_ADDRESSES: Record<TronNetwork, JustLendAddresses> = {
  [TronNetwork.Mainnet]: {
    ...
    stUSDT: {
      ...
      wstUsdt: "TGkxzkDKyMeq2T7edKnyjZoFypyzjkkssq",
      minter: "TVh1PF9xr4zC5uAqRcCbxF1By6ucp95G4i",
      unstaker: "TURYwFtG6gvpEyPSm55FyjJWpgQQ2rDm5e",
      decimals: 18,
      usdtDecimals: 6,
    },
  ...
  [TronNetwork.Nile]: {
    ...
    stUSDT: {
      ...
      stUsdt: "TVUGRzuUBoUmFvuHFfgyrFS39PDtEDHfX9",
      wstUsdt: "TQuaRvcTVquWNKWGiA4zVgcy1ChXNX7p54",
      minter: "TJNuCgiy68xssHojMSnjt86RbzFBmFqREr",
      unstaker: "TVqzeEEX1qkNr4jxZCo8g3xbSdBForBR7n",
      decimals: 18,
      usdtDecimals: 6,
    },
    ...
    jTokens: {
      ...
      jUSDT: {
        ...
        underlying: "TPYwAC9Y4uUcT2QH3WPPjqxzJSJWymMoMS",  // 🔧 Fix: originally TXYZopYRdj2D9XRtbG411XZZ3kM5VkAeBf
        symbol: "jUSDT",
        underlyingSymbol: "USDT",
        decimals: 8,
        underlyingDecimals: 6,
      ...
      jSUN: {
        ...
        underlying: "TESJCkrX1rrNgJNb69b4vUJzSNBn1B8iZC",  // 🔧 Fix: originally TDqjTkZ63yHB19w2n7vPm2qAkLHwn9fKKk
        symbol: "jSUN",
        underlyingSymbol: "SUN",
        decimals: 8,
        underlyingDecimals: 18,
      ...
      jBTT: {
        ...
        underlying: "TBagxx57zx73VJJ61o12VfxzQ2EG3KHYJp",  // 🔧 Fix: originally TVSvjZdyDSNocHm7dP3jvCmMNsCnMTPa5W
        symbol: "jBTT",
        underlyingSymbol: "BTT",
        decimals: 8,
        underlyingDecimals: 18,
      ...
      jETHB: {
        ...
        underlyingSymbol: "ETHB",
        decimals: 8,
        underlyingDecimals: 18,                              // 🔧 Fix: originally 6, API shows collateralDecimal=18
      },
      jHTX: {                                                // 🆕 New market
      ...
      jUSDCOLD: {                                            // 🔧 Rename: originally jUSDC, aligned with USDCOLD from API
        address: "TMsoCkr2yhukcGnvjhVk8Gj541BCQPEHwm",
        underlying: "TM1Xq1HHd5RTcR4VAiQ8oV6CQvfVdn3F1f",  // 🔧 Fix: originally TWMCMCoJPqCGw5RR7eChF2HoY3a9B8eYA3
        symbol: "jUSDCOLD",
        underlyingSymbol: "USDCOLD",
        decimals: 8,
        underlyingDecimals: 6,
      ...
      jsTRX2: {                                              // 🆕 New: second sTRX market (id=21)
    ...
    // See Nile write path documentation and integration tests for details.
    moolah: {
...
export function getApiHost(network: string = DEFAULT_NETWORK): string {
  if (process.env.JUSTLEND_API_HOST) return validateTrustedUrl(process.env.JUSTLEND_API_HOST, 'apiHost');
  const n = network.toLowerCase();
  if (n === "mainnet" || n === "tron" || n === "trx") return validateTrustedUrl(JUSTLEND_API_HOSTS.mainnet, 'apiHost');
  if (n === "nile" || n === "testnet") return validateTrustedUrl(JUSTLEND_API_HOSTS.nile, 'apiHost');
  return validateTrustedUrl(JUSTLEND_API_HOSTS.mainnet, 'apiHost');
}
...
export function getMoolahApiHost(network: string = DEFAULT_NETWORK): string {
  if (process.env.JUSTLEND_MOOLAH_API_HOST) return validateTrustedUrl(process.env.JUSTLEND_MOOLAH_API_HOST, 'moolahApiHost');
  const n = network.toLowerCase();
  if (n === "mainnet" || n === "tron" || n === "trx") return validateTrustedUrl(MOOLAH_API_HOSTS.mainnet, 'moolahApiHost');
  throw new Error(
