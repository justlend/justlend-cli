 ...
 * Host resolved via getApiHost(network) (mainnet only — these endpoints do not
 * exist on nile) and re-checked through validateTrustedUrl so records/rewards
 * share the same trusted-host allowlist + JUSTLEND_API_HOST override as api/v1.
 *
...
async function fetchRecords<T>(
  ...
  const url = validateTrustedUrl(
    `${host}${path}?addr=${encodeURIComponent(addr)}&page=${page}&pageSize=${pageSize}`,
    'apiHost',
  );
...
export async function fetchLendingRecords(
  ...
  const raw = await fetchRecords<LendingRecord>("/justlend/record/depositBorrow", address, page, pageSize, network);
...
export async function fetchStrxRecords(
  ...
  requireMainnet(network);
  const raw = await fetchRecords<StrxRecord>("/justlend/record/strx", address, page, pageSize, network);
...
export async function fetchVoteRecords(
  ...
  requireMainnet(network);
  const raw = await fetchRecords<VoteRecord>("/justlend/record/vote", address, page, pageSize, network);
...
export async function fetchEnergyRentalRecords(
  ...
  requireMainnet(network);
  const raw = await fetchRecords<EnergyRentalRecord>("/justlend/record/rent", address, page, pageSize, network);
...
export async function fetchLiquidationRecords(
  ...
  requireMainnet(network);
  return fetchRecords<LiquidationRecord>("/justlend/record/liquidate", address, page, pageSize, network);
}
...
export async function fetchClaimableRewards(
  ...
  const url = validateTrustedUrl(
    `${getApiHost(network)}/sunProject/getAllUnClaimedAirDrop?addr=${encodeURIComponent(address)}`,
    'apiHost',
  );
  const res = await fetchWithTimeout(url);
